...
что задача оценки параметров движения для всех ВС, находящихся в зоне наблюдения, 
...
со вновь поступающими не привязанными к конкретному ВС измерениями. 
...
$\varsigma_l$ --- вектор параметров, характеризующий состояние РЛС; $w_l(t)$ --- текущая реализация 
...
в которых переменные, относящиеся к разным движениям и систематическим ошибкам, 
...
но одинаковому по структуре уравнению движения. 
...
Подробно понятия систематических ошибок по дальности и азимуту введены в отчёте \cite{bibNitaOtchet2012-07}. 
...
При этом будем считать, что такое измерение одно и в момент $t_k$ нет никаких других измерений. 
...
\begin{gather}
  ...
  %\begin{bmatrix}
    %0 & \cdots & 0 & C^\chi_i(t_k) & 0 & \cdots & 0 
  %\end{bmatrix}
  \,, \notag \\ 
  ...
  %\begin{bmatrix}
    %0 & \cdots & 0 & C^\varsigma_j(t_k, \chi_i(t_k)) & 0 & \cdots & 0 
  %\end{bmatrix}
  \,, \notag \\ 
...
Его целью является получение новой оценки, учитывающий последнее измерение. 
...
Популярным дополнительным условием является {\it условие несмещённости} оценки 
\begin{gather}
...
Далее, все алгоритмы будут рассматриваться с условием \eqref{Bedin:eq:est-unbiased-L} 
...
фигурирует разность между действительным и прогнозным измерениями. 
...
тогда для произвольного матричного коэффициента $K$ (не зависимо от способа получения) справедливо 
\begin{multline*}
  \hat{\xi}(t_k) - \xi(t_k) = 
  \bar{\xi}(t_k) - \xi(t_k) + 
...
\end{multline}
известное как {\it формула Йозефа} \cite{BarShalomBiasEst2013}. 
...
называются уравнениями фильтра Калмана (или рекуррентной оценки Гаусса-Маркова для случая $B = 0$). 
...
\section{Упрощенные алгоритмы оценивания по Henk Blom}
...
В этом варианте рассматривается ситуация, когда для каждого движения $i$ 
в некоторый момент $t_k$ одновременно приходят измерения нескольких радиолокаторов. 
...
отличен от нуля только для тех переменных, 
которые непосредственно участвуют в уравнении наблюдения для текущего измерения. 
Это приводит к идее, что следует находить оптимальный фильтр, заранее удовлетворяющий такому свойству, 
которое далее выступает как некоторое ограничение в задаче оптимизации. 

Пусть в рассматриваемый момент времени $t_k$ движение с номером $i$ 
измеряется радиолокатором с номером $j$. Уравнение наблюдения имеет вид 
\begin{gather*}
  z(t_k) = C^{\chi}_i \chi_i(t_k) + C^{\varsigma}_j \varsigma_j(t_k) + D_j w_j \,. 
\end{gather*}
...
Новые матрицы могут быть получены из старых путём перестановок строк и столбцов. 
%простых линейных преобразований. 
...
Рассматриваем коэффициенты $K$ фильтра следующего вида 
\begin{gather}
  K = 
  \begin{bmatrix}
    K_{\chi_i} \\ 
...
в которых каждая строка $h^\mathsf{T}$ выделяет лишь переменные, 
относящиеся к какому-нибудь одному набору параметров --- либо $\chi_i$, либо $\varsigma_j$. 
...
что приводит к тем же самым неприятным эффектам, что и оптимальная фильтрация при помощи фильтра Калмана. 
Для параллельного применения алгоритма требуется устранить влияние 
кросс-ковариаций $\bar{P}_{\chi_i \varsigma_j}$ на вычисления $K_{\chi_i}$ 
и при этом обеспечить не очень большое <<загрубление>>. 

Рассмотрим блоки матрицы $\bar{P}$, участвующие в выражении для блока $\hat{P}_{\chi_i \chi_i}$. 
Легко проверить следующее матричное неравенство:  
\begin{gather*}
  \begin{bmatrix}
    \bar{P}_{\chi_i \chi_i}       & \bar{P}_{\chi_i \varsigma_j}      \\ 
    \bar{P}_{\varsigma_j \chi_i}  & \bar{P}_{\varsigma_j \varsigma_j}  
  \end{bmatrix}
  \preccurlyeq 
  \begin{bmatrix}
    (1 + \gamma) \bar{P}_{\chi_i \chi_i} & 0 \\ 
    0 & \left( 1 + \frac{1}{\gamma} \right) \bar{P}_{\varsigma_j \varsigma_j}  
  \end{bmatrix}
\end{gather*}
при любом выборе $\gamma > 0$. Знак $\preccurlyeq$ означает порядок для симметричных матриц 
в смысле неотрицательной определённости (разность матрицы справа и слева неотрицательно определена). 
Это матричное неравенство приводит к новому матричному неравенству для $\hat{P}_{\chi_i \chi_i}$: 
\begin{multline}
  \hat{P}_{\chi_i \chi_i} 
  \preccurlyeq 
  (1 + \gamma) 
  ...
  K_{\chi_i} D_j W_j D_j^\mathsf{T} K_{\chi_i}^\mathsf{T}
  \,. 
  \label{Bedin:eq:CI-Pxx}
\end{multline}
В работе \cite{CI2015} показано, что неравенство является существенно нестрогим, 
т.~е. равенство достигается <<на некоторых направлениях>> в пространстве матриц, 
и неравенство не может быть заменено на строгое. 

Опираясь на оценку \eqref{Bedin:eq:CI-Pxx}, легко предложить следующий алгоритм фильтрации. 
Пусть для момента времени $t_{k - 1}$ выполнено 
\begin{align*}
  \hat{P}_{\chi_i \chi_i}(t_{k - 1})  & \preccurlyeq \hat{R}_{\chi_i \chi_i}(t_{k - 1}) \,, \\ 
  \hat{P}_{\varsigma_j \varsigma_j}(t_{k - 1}) & \preccurlyeq \hat{R}_{\varsigma_j \varsigma_j}(t_{k - 1}) \,.  
\end{align*}
Тогда, основываясь на следующем свойстве 
\begin{gather*}
  Q \preccurlyeq U \; \Longrightarrow\ \; T Q T^\mathsf{T} \preccurlyeq T U T^\mathsf{T} \, 
\end{gather*}
для произвольных неотрицательно определённых матриц $Q$, $U$ и произвольной матрицы $T$, 
а также на виде прогнозной матрицы ковариации \eqref{Bedin:eq:est-cov-pred-evolution}, 
можно утверждать, что и  
\begin{align*}
  \bar{P}_{\chi_i \chi_i}(t_k) 
  & \preccurlyeq 
  A_i \hat{R}_{\chi_i \chi_i}(t_{k - 1}) A_i^\mathsf{T} + 
  B_i V^\chi_i B_i^\mathsf{T} 
  \triangleq 
  \bar{R}_{\chi_i \chi_i}(t_k) 
  \,, \\ 
  \bar{P}_{\varsigma_j \varsigma_j}(t_k) 
  & \preccurlyeq 
  A_j^\varsigma \hat{R}_{\varsigma_j \varsigma_j}(t_{k - 1}) A_j^{\varsigma \, \mathsf{T}} + 
  B_j^\varsigma V_j^\varsigma B_j^{\varsigma \, \mathsf{T}} 
  \triangleq 
  \bar{R}_{\varsigma_j \varsigma_j}(t_k) 
  \,. 
\end{align*}
Следовательно, %после воздействия коэффициента $K_{\chi_i}$ 
для блока $\hat{P}_{\chi_i \chi_i}$ будет справедливо: 
\begin{multline}
  \hat{P}_{\chi_i \chi_i}(t_k) 
  \preccurlyeq 
  (1 + \gamma) 
  ...
  \left( 1 + \frac{1}{\gamma} \right)
  K_{\chi_i} C^\varsigma_j
  \bar{R}_{\varsigma_j \varsigma_j} 
  C^{\varsigma \, \mathsf{T}}_j K_{\chi_i}^\mathsf{T} 
  + 
  K_{\chi_i} D_j W_j D_j^\mathsf{T} K_{\chi_i}^\mathsf{T}
  \,. 
  \label{Bedin:eq:CI-Pxx-est}
\end{multline}
Далее, матрицу в правой части \eqref{Bedin:eq:CI-Pxx-est} 
можно выбрать исходя из минимума некоторого критерия, например следа, 
при этом получив новую оценку $\hat{R}_{\chi_i \chi_i}$. 
При минимизации следа путём варьирования $K_{\chi_i}$ получаются следующие соотношения: 
\begin{align}
  S^* 
  & = 
  (1 + \gamma) 
  C^\chi_i \bar{R}_{\chi_i \chi_i}(t_k) C^{\chi \, \mathsf{T}}_i 
  + 
  \left( 1 + \frac{1}{\gamma} \right)
  C^\varsigma_j \bar{R}_{\varsigma_j \varsigma_j}(t_k) C^{\varsigma \, \mathsf{T}}_j  
  + 
  D_j W_j D_j^\mathsf{T} 
  \,, \notag \\ 
  K^*_{\chi_i} 
  & = 
  (1 + \gamma) 
  \bar{R}_{\chi_i \chi_i} C^{\chi \, \mathsf{T}}_i S^{* \, \mathsf{-1}} 
  \,, \notag \\ 
  \hat{R}_{\chi_i \chi_i}(t_k) 
  & = 
  (1 + \gamma) 
  \left( 
    \bar{R}_{\chi_i \chi_i}(t_k) - 
    (1 + \gamma) 
    \bar{R}_{\chi_i \chi_i} C^{\chi \, \mathsf{T}}_i S^{* \, \mathsf{-1}} 
    C^\chi_i \bar{R}_{\chi_i \chi_i}(t_k) 
  \right)
  = \label{Bedin:eq:CI-Rxx-est} \\ & = 
  (1 + \gamma) \bar{R}_{\chi_i \chi_i}(t_k) - 
  K^*_{\chi_i} S^* K^{* \, \mathsf{T}}_{\chi_i}
  \notag \, 
\end{align}
с условием 
\begin{gather}
  \hat{P}_{\chi_i \chi_i}(t_k) \preccurlyeq \hat{R}_{\chi_i \chi_i}(t_k) \,. 
  \label{Bedin:eq:CI-Rxx-condition}
\end{gather}
Соотношения \eqref{Bedin:eq:CI-Rxx-est} справедливы для произвольного $\gamma > 0$. 
С помощью выбора $\gamma$ также можно улучшить качество аппроксимации, 
минимизируя след $\hat{R}_{\chi_i \chi_i}$. 

Аналогично можно вывести соотношения на оценку для $\varsigma_j$ и коэффициент фильтра $K_{\varsigma_j}$. 
При этом можно пользоваться новой константой $\gamma$, выбранной независимо от предыдущей. 
В итоге получается алгоритм оценивания, который работает гарантированно 
в смысле неравенства \eqref{Bedin:eq:CI-Rxx-condition} 
для произвольных матриц кросс-ковариаций $\bar{P}_{\chi \varsigma}$. 
В качестве дополнительных параметров, по которым производится вычисление 
\eqref{Bedin:eq:recursive-est-add-par} матричных коэффициентов фильтра, 
выступают матрицы $\bar{R}_{\chi_i \chi_i}$, $\hat{R}_{\chi_i \chi_i}$, 
$\bar{R}_{\varsigma_j \varsigma_j}$, $\hat{R}_{\varsigma_j \varsigma_j}$ 
для всех $i = 1, \ldots, n$, $j = 1, \ldots, m$.  

По видимому, впервые алгоритм, основанный на соотношениях, близких к 
\eqref{Bedin:eq:CI-Pxx-est}, \eqref{Bedin:eq:CI-Rxx-est}, 
был предложен в работе \cite{CI1997}, в дальнейшем он получил название CI --- Covariance Intersection. 
Отметим, что полное обоснование корректности его работы было получено лишь недавно, 
в цикле статей Pablo Arambel и Uwe Hanebeck, смотреть следует статью~\cite{CI2015}. 

\section{Моделирование работы алгоритмов параллельной фильтрации}

Было произведено численное моделирование алгоритмов фильтрации. 
Рассматривались три алгоритма: фильтр Калмана (в качестве контрольного эталонного алгоритма), 
разделённый фильтр по Henk Blom и фильтр, основанный на CI. 

В качестве модельной задачи была взята задача наблюдения за воздушными судами на плоскости. 
Использовалась простая динамика ВС в виде прямолинейного и равномерного движения 
\eqref{Bedin:eq:x-straight-line-dynamics}. 
Модельные траектории представляют собой набор из 15 движений, 
начальные условия для которых были выбраны случайно. 
Общий вид движений приведён на рис.~\ref{Bedin:pic:traj}. 
Длительность движений по времени составляет 1000 с. 

Наблюдающие радиолокаторы обладают постоянными систематическими ошибками по дальности и азимуту 
и располагаются в заданных точках на плоскости. Их характеристики приведены ниже в таблице 
\begin{table*}[ht]%
\begin{tabular}{|l|l|l|l|}
  ...
  \hline 
  \hline 
  координаты, м, С & -20000 & 230000 & -35000 \\
  $\phantom{\mbox{координаты, м,}}$ В & -20000 & -30000 & 235000 \\
  сист. ошибка по азимуту, ${}^\circ$ 
  & -0.19 & -0.16 & 0.17 \\
  сист. ошибка по дальности, м 
  & 400 & 130 & -300 \\
  скорость дрейфа сист. ошибки по азимуту, ${}^\circ/c$ 
  & 0.02 & 0.02 & 0.02 \\
  скорость дрейфа сист. ошибки по дальности, м/c 
  & 1 & 1 & 1 \\
  случ. ошибка по азимуту, ${}^\circ$ 
  & 0.05 & 0.05 & 0.05 \\
  случ. ошибка по дальности, м 
  & 15 & 15 & 15 \\
  \hline
\end{tabular}
\end{table*}
Измерения производились с интервалом 1 раз в секунду. 
При этом все радиолокаторы одновременно наблюдали все движения. 

Моделирование производилось в несколько итераций, представлены результаты для 20 из них. 
На каждой итерации формировались свои реализации случайных ошибок измерений и случайных ошибок движения, 
при этом траектории движения ВС и систематические ошибки оставались неизменными. 
Далее, на рисунках представлены результаты работы алгоритмов. 

На рис.~\ref{Bedin:pic:kf-dx}--\ref{Bedin:pic:kf-dr} представлены результаты работы фильтра Калмана. 
На рис.~\ref{Bedin:pic:blom-dx}--\ref{Bedin:pic:blom-dr} показано качество оценивания алгоритма 
по статье Henk Blom. 
Рис.~\ref{Bedin:pic:ci-dx}--\ref{Bedin:pic:ci-dr} относятся к алгоритму, в основе которого лежит метод CI. 
На всех рис. изображены графики зависимости от времени для 
модулей отклонения между истинными параметрами движения и их оценками, 
а также теоретические уровни такого отклонения. 
Рассматриваются следующие характеристики: 
на рис.~\ref{Bedin:pic:kf-dx},~\ref{Bedin:pic:blom-dx},~\ref{Bedin:pic:ci-dx} 
показаны отклонения для вектора геометрического положения, рассчитывается модуль отклонения в метрах; 
на рис.~\ref{Bedin:pic:kf-dv},~\ref{Bedin:pic:blom-dv},~\ref{Bedin:pic:ci-dv} --- 
отклонения для вектора скорости, рассчитывается модуль отклонения в м/с; 
на рис.~\ref{Bedin:pic:kf-da},~\ref{Bedin:pic:blom-da},~\ref{Bedin:pic:ci-da} --- 
отклонения для систематической ошибки по азимуту, рассчитывается модуль отклонения в ${}^\circ$; 
на рис.~\ref{Bedin:pic:kf-dr},~\ref{Bedin:pic:blom-dr},~\ref{Bedin:pic:ci-dr} --- 
отклонения для систематической ошибка по дальности, рассчитывается модуль отклонения в метрах. 

На всех рис. синим цветом показан модуль отклонения рассматриваемой величины 
для одной выделенной итерации моделирования. 
Чёрной пунктирной кривой показан график для усреднённого по всем 20 реализациям модуля отклонения. 
Красным показан график теоретической оценки отклонения параметра, сделанной при помощи 
части следа истинной матрицы ковариаций, пересчитываемой по уравнению \eqref{Bedin:eq:est-cov-corr-evolution}. 
Методика рассчёта связана с тем, что в формуле \eqref{Bedin:eq:crit} берётся $h^\mathsf{T}$, 
выделяющий нужные переменные, а затем используется равенство 
\eqref{Bedin:eq:crit} и \eqref{Bedin:eq:crit-trace}. 
Зелёным цветом показана теоретическая оценка отклонения, сделанная при помощи 
части следа матрицы ковариации, использующейся внутри алгоритма. 
Для алгоритма по Henk Blom это части пересчитываемой приближённо $\hat{P}$. 
Для алгоритма, основанного на CI --- части оценки $\hat{R}$. 

Группа рис.~\ref{Bedin:pic:all-dx}--\ref{Bedin:pic:all-dr} 
сделана для простого сравнения работы алгоритмов. 
На каждом рисунке приведены оценки качества для разных алгоритмов, 
вычисленные для одной и той же характеристики. 
Оценка качества берётся в виде усреднённого по всем 20 реализациям модуля отклонения. 
На рис.~\ref{Bedin:pic:all-dx} изображены отклонения для геометрического положения, 
на рис.~\ref{Bedin:pic:all-dv} --- для вектора скорости, 
на рис.~\ref{Bedin:pic:all-da} --- для систематической ошибки по азимуту, 
на рис.~\ref{Bedin:pic:all-dr} --- для систематической ошибки по дальности. 

На основании моделирования можно сделать следующие выводы. 
Оба алгоритма распараллеливания показали неплохие результаты, но совершенно по-разному. 
Так, алгоритм на основе CI показал лучшую точность (даже по сравнению с фильтром Калмана) 
для вычисления скорости и систематических ошибок по азимуту, 
однако обладает очень долгим переходным процессом вследствие своей <<консервативности>>. 
Алгоритм рассчитан на произвольные кросс-ковариации между частями фазового вектора, 
должен работать для всех возможных частных случаев и поэтому 
имеет маленькие по модулю компоненты коэффициента усиления $K$. 
Алгоритм Henk Blom по большинству показателей работает сравнимо с алгоритмом фильтрации Калмана, 
однако также имеет завышенное время переходного процесса. 

\begin{figure}[ht]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/traj.eps}%
\vspace{-4mm}
\caption{Модельные траектории и радиолокаторы. Положения радиолокаторов покзаны треугольниками}%
\label{Bedin:pic:traj}%
\end{figure}
\clearpage
...
\begin{figure}[ht]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/kf-dx.eps}%
\vspace{-4mm}
\caption{Фильтр Калмана. Отклонение по вектору положения показано в метрах}%
\label{Bedin:pic:kf-dx}%
\end{figure}
\begin{figure}[hb]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/kf-dv.eps}%
\vspace{-4mm}
\caption{Фильтр Калмана. Отклонение по вектору скорости показано в м/с}%
\label{Bedin:pic:kf-dv}%
\end{figure}
\clearpage
...
\begin{figure}[ht]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/kf-da.eps}%
\vspace{-4mm}
\caption{Фильтр Калмана. Отклонение для систематической ошибки по азимуту показано в ${}^\circ$}%
\label{Bedin:pic:kf-da}%
\end{figure}
\begin{figure}[hb]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/kf-dr.eps}%
\vspace{-4mm}
\caption{Фильтр Калмана. Отклонение для систематической ошибки по дальности показано в метрах}%
\label{Bedin:pic:kf-dr}%
\end{figure}
\clearpage
...
\begin{figure}[ht]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/blom-dx.eps}%
\vspace{-4mm}
\caption{Упрощённый фильтр H. Blom. Отклонение по вектору положения показано в метрах}%
\label{Bedin:pic:blom-dx}%
\end{figure}
\begin{figure}[hb]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/blom-dv.eps}%
\vspace{-4mm}
\caption{Упрощённый фильтр H. Blom. Отклонение по вектору скорости показано в м/с}%
\label{Bedin:pic:blom-dv}%
\end{figure}
\clearpage
...
\begin{figure}[ht]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/blom-da.eps}%
\vspace{-4mm}
\caption{Упрощённый фильтр H. Blom. Отклонение для систематической ошибки по азимуту показано в ${}^\circ$}%
\label{Bedin:pic:blom-da}%
\end{figure}
\begin{figure}[hb]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/blom-dr.eps}%
\vspace{-4mm}
\caption{Упрощённый фильтр H. Blom. Отклонение для систематической ошибки по дальности показано в метрах}%
\label{Bedin:pic:blom-dr}%
\end{figure}
\clearpage
...
\begin{figure}[ht]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/ci-dx.eps}%
\vspace{-4mm}
\caption{Упрощённый фильтр на основе метода CI. Отклонение по вектору положения показано в метрах}%
\label{Bedin:pic:ci-dx}%
\end{figure}
\begin{figure}[hb]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/ci-dv.eps}%
\vspace{-4mm}
\caption{Упрощённый фильтр на основе метода CI. Отклонение по вектору скорости показано в м/с}%
\label{Bedin:pic:ci-dv}%
\end{figure}
\clearpage
...
\begin{figure}[ht]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/ci-da.eps}%
\vspace{-4mm}
\caption{Упрощённый фильтр на основе метода CI. Отклонение для систематической ошибки по азимуту показано в ${}^\circ$}%
\label{Bedin:pic:ci-da}%
\end{figure}
\begin{figure}[hb]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/ci-dr.eps}%
\vspace{-4mm}
\caption{Упрощённый фильтр на основе метода CI. Отклонение для систематической ошибки по дальности показано в метрах}%
\label{Bedin:pic:ci-dr}%
\end{figure}
\clearpage
...
\begin{figure}[ht]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/all-dx.eps}%
\vspace{-4mm}
\caption{Сравнение разных фильтров. Отклонение по вектору положения показано в метрах}%
\label{Bedin:pic:all-dx}%
\end{figure}
\begin{figure}[hb]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/all-dv.eps}%
\vspace{-4mm}
\caption{Сравнение разных фильтров. Отклонение по вектору скорости показано в м/с}%
\label{Bedin:pic:all-dv}%
\end{figure}
\clearpage
...
\begin{figure}[ht]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/all-da.eps}%
\vspace{-4mm}
\caption{Сравнение разных фильтров. Отклонение для систематической ошибки по азимуту показано в ${}^\circ$}%
\label{Bedin:pic:all-da}%
\end{figure}
\begin{figure}[hb]%
\centering
\includegraphics[width=0.8\columnwidth]{part1pict/all-dr.eps}%
\vspace{-4mm}
\caption{Сравнение разных фильтров. Отклонение для систематической ошибки по дальности показано в метрах}%
\label{Bedin:pic:all-dr}%
\end{figure}
\clearpage
